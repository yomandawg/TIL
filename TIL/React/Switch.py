...
class Switch(StpSwitch):
    ...
    def process_message(self, message):
        ...
        if message.root < self.root: # valid
            self.root = message.root
            self.dist = message.distance + 1
            self.switchThrough = message.origin
            self.activeLinks = []
            self.activeLinks.append(message.origin)
            msg = Message(self.root, self.dist, self.switchID, message.origin, True)
            self.send_message(msg)
        
        elif message.root > self.root: # invalid
            msg = Message(self.root, self.dist, self.switchID, message.origin, False)
            self.send_message(msg)
        
        elif message.root == self.root:
            if (message.distance + 1 < self.dist) or (message.distance == self.dist):
                self.dist = message.distance + 1
                self.switchThrough = message.origin
    ...
    def generate_logstring(self):
        ...
        for link in self.activeLinks:
            temp = str(self.switchID) + ' - ' + str(link) + ', '
            log += temp
